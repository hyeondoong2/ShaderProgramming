...
void Renderer::CreateVertexBufferObjects()
{
	...
	glBufferData(GL_ARRAY_BUFFER, sizeof(testPos),
		testPos, GL_STATIC_DRAW);
...
void Renderer::DrawTest()
{
	...
	int aPosLoc = glGetAttribLocation(m_TestShader,
		"a_Position");
	int aColLoc = glGetAttribLocation(m_TestShader,
		"a_Color");
	int aRadiusLoc = glGetAttribLocation(m_TestShader,
		"a_Radius");
...
void Renderer::DrawParticle()
{
	...
	int aPosLoc = glGetAttribLocation(shader,
		"a_Position");
	int aColLoc = glGetAttribLocation(shader,
		"a_Color");
	int aValueLoc = glGetAttribLocation(shader,
		"a_Value");
	int aSTimeLoc = glGetAttribLocation(shader,
		"a_STime");
	int aVelocity = glGetAttribLocation(shader,
		"a_Velocity");
	int aLifetime = glGetAttribLocation(shader,
		"a_Lifetime");
	int aMassLoc = glGetAttribLocation(shader,
		"a_Mass");
	int aPeriodLoc = glGetAttribLocation(shader,
		"a_Period");
	...
	glVertexAttribPointer(
		aVelocity, 3, GL_FLOAT,
		GL_FALSE, sizeof(float) * stride,
		(GLvoid*)(sizeof(float) * 9));

	glVertexAttribPointer(
		aLifetime, 1, GL_FLOAT,
		GL_FALSE, sizeof(float) * stride,
...
void Renderer::GenerateParticles(int numParticles)
{
	...
	for (int i = 0; i < numParticles; ++i) {
		...
		float vx, vy, vz;
		vx = (((float)rand() / (float)RAND_MAX) * 2.f - 1.f) * 1.5;
		vy = ((((float)rand() / (float)RAND_MAX) * 2.f - 1.f) + 1.5f) * 1.5;
		vz = 0.f;
